...
void stack_from_queues_push(struct stack_from_queues* stack, int value) {
	assert(stack);
	// new value will be placed in empty queue (q2) ahead of other values
	queue_enqueue(stack->q2, value);
	// moving the rest of the stack behind the new value
	while (!queue_isempty(stack->q1)) {
		queue_enqueue(stack->q2, queue_dequeue(stack->q1));	
	}
	// moving the queue back to the accessible queue (q1)
